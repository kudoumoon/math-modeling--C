...
def audit(path: Path, question: str) -> dict:
    ...
    checks["interval_start_mapping_A"] = bool(np.array_equal(pd.to_datetime(frame.interval_start), starts))
    checks["interval_end_mapping_A"] = bool(np.array_equal(pd.to_datetime(frame.interval_end), starts + pd.Timedelta(minutes=10)))
