    ...
    for page in reader.pages:
        try:
            ...
            for fname, fobj in fonts.items():
                ...
                # Matplotlib TrueType PDFs use Type0 fonts whose descriptors
                # belong to descendant CID fonts, not the Type0 parent.
                descendants = font.get("/DescendantFonts")
                embedding_fonts = [ref.get_object() for ref in descendants] if descendants else [font]
                embedded = True
                for embedding_font in embedding_fonts:
                    descriptor = embedding_font.get("/FontDescriptor")
