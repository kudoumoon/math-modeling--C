...
2026-09-13流程更新：按用户授权，暂存Q2汇总P2的RESET1边界审核问题，优先重建Q3 v3和Q4 v1。新版本以已完成的因果Q2主运行为显式暂定依赖，依次执行Q3、Q4，再独立联合审核和一起修订；每版执行一次，省略重复全年复现。新旧目录严格区分，当前未签发最终正式结果。详见[联合迭代规约](reports/q3_q4_joint_iteration_contract.md)与机器状态中的 `joint_iteration_override`。
...
不在本次冻结范围内：社交平台方案包、Python 虚拟环境、缓存和重复 ZIP。新版本以 `program/results/figures/others` 分类；`WRONG_UPSTREAM_ARCHIVE` 中的输出只供历史审计。

## 最重要的时间口径

- 当前主运行采用Time A：每日144个十分钟区间按 `0:10–0:20` 开始，最后以次日 `0:00–0:10+1` 结束。这是当前建模假设，附件端点歧义仍保留；新Q3/Q4未新增Time B或一槽延迟敏感性。
...
当前结果口径：Q1 已归档但仍待重新冻结；Q2 ONLINE-RISK V2 是条件数值基线，Q2 v3主运行已完成但P2汇总按用户指令暂缓。新Q3/Q4以该因果主运行为显式暂定依赖，保持 `formal_use=false`；旧错误上游归档不能作为正式结果。
